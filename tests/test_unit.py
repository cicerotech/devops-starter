...
async def test_create_book_unit(mock_db):
    # Arrange
    book_data = BookCreate(title="Test Book", author="Test Author", price=29.99)
...
async def test_list_books_unit(mock_db):
    # Arrange
    mock_books = [
        Book(id=1, title="Book 1", author="Author 1", price=19.99, 
             created_at=datetime.now()),
        Book(id=2, title="Book 2", author="Author 2", price=29.99, 
             created_at=datetime.now())
    ]
    mock_result = Mock()
...
async def test_get_book_unit(mock_db):
    # Arrange
    mock_book = Book(id=1, title="Test Book", author="Test Author", 
                    price=29.99, created_at=datetime.now())
    mock_result = Mock()
